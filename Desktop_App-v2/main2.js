 ...
 * ERROR: Cannot find route somewhere
 */
let flaskProc=null;
const createFlaskProc = () => {
    //dev
    const activateScript = path.join(backendDirectory, "routes", "run")
    let activateVenv;
    let command;
    let args;
    if (process.platform == "win32") {
        ...
        command = "cmd";
        args = ["/c", `${activateVenv} && python -m flask --app ${activateScript} --debug run`]
    } else {    //Mac or Linux
        ...
        args = ["-m", "flask", "--app", activateScript, "run"];
    }
...
const shutdownFlaskConnection = () => {
    ...
    */
    return new Promise((resolve, reject) => {
        ...
        fetch(`${SERVER_URL}/shutdown`, { method:"GET" })
        .then((response) => {
            if (!response.ok) {
