...
def test_set_curr_options_adds_only_unique_new_options_to_current_question(test_client:FlaskClient, mocker:MockerFixture):
...
def clear_session_options_clears_current_options_session_variable(test_client:FlaskClient, test_session):
    curr_options = ["1", "2", "3", "4"]
    expected = []
    sess_var = {"curr_options": curr_options}
    test_session(test_client, sess_var)

    response = test_client.delete("/clear_session_options")
    assert response.status_code == 200
    result = response.get_json()
