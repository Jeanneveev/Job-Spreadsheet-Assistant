...
def set_curr_options():
    """Adds the given options to the list of current options"""
    ...
    curr_options = session.get("curr_options", [])
    curr_options.extend(options)
    session["curr_options"] = curr_options
...
@opt_bp.route("/clear_current_options", methods=["DELETE"])
